...
let rec format_string = function
  ...
  | LetRec (def, k) ->
    Printf.sprintf "@[<v 0>LetRec (@[<0>%s: %s,@ %s,@ %s,@]@ %s)@]"
      ...
      (format_string k)
  | App (id, idlist) ->
    binary "App" (quoted id) (format_string_of_list idlist (quoted))
  ...
  | Put (id1, id2, id3) ->
    ternary "Put" (quoted id1) (quoted id2) (quoted id3)
  | ExtArray id -> unary "ExtArray" (quoted id)
  | ExtFunApp (id, idlist) ->
    binary "ExtFunApp" (quoted id) (format_string_of_list idlist (quoted))
